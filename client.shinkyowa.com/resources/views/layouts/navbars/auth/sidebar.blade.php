<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-left ms-3" id="sidenav-main">
    <div class="sidenav-header">
        ...
        <a class="navbar-brand m-0" href="{{ route('dashboard') }}">
            ...
            <span class="ms-1 font-weight-bold">SHINKYOWA</span>
        </a>
    </div>
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            @if(auth()->user()->roles->pluck('name') == '["client"]')
                ...
                <li class="nav-item pb-2">
                    <a class="nav-link {{ Route::currentRouteName() == 'current-orders' ? 'active' : '' }}"
                        href="{{ route('current-orders') }}">
                        <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <i style="font-size: 1rem;" class="ni ni-bag-17 ps-2 pe-2 text-center
                            {{ in_array(request()->route()->getName(),['current-orders']) ? 'text-white' : 'text-dark' }}"></i>
                        </div>
            ...
            @if(auth()->user()->roles->pluck('name') == '["admin"]')
                ...
                <li class="nav-item pb-2">
                    <a class="nav-link {{ Route::currentRouteName() == 'add-vehicle' ? 'active' : '' }}"
                        href="{{ route('add-vehicle') }}">
                        <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <i style="font-size: 1rem;" class="ni ni-bus-front-12 ps-2 pe-2 text-center
                            {{ in_array(request()->route()->getName(),['add-vehicle']) ? 'text-white' : 'text-dark' }}"></i>
                        </div>
                ...
                <li class="nav-item pb-2">
                    <a class="nav-link {{ Route::currentRouteName() == 'all-orders' ? 'active' : '' }}"
                        href="{{ route('all-orders') }}">
                        <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <i style="font-size: 1rem;" class="ni ni-bag-17 ps-2 pe-2 text-center
                            {{ in_array(request()->route()->getName(),['all-orders']) ? 'text-white' : 'text-dark' }}"></i>
                        </div>
